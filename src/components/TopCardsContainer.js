        ...
        <div className="d-flex flex-row styless justify-content-center m-4">
            <MoveButton handleMove={handleBackData} goBack={true}/>
             {topData().slice(dataStart,dataStart + carouselLength).map(record => 
                isMovie ? <div key={record.id} className="top-card carousel"><MovieCard movie={record} styleName={"resource-card"}/></div> :
                <div key={record.id} className="top-card carousel"><ReviewerCard reviewer={record} styleName={"resource-card"}/></div>)}
